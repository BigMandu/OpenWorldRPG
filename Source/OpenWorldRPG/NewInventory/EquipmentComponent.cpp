...
UItemStorageObject* UEquipmentComponent::GetEquipStorage(EEquipmentType Type)
{
	switch (Type)
	{
...
bool UEquipmentComponent::AddEquipment(FItemSetting ItemSetting, AEquipment* WantToEquip)
{
	...
	if (EquipObj && CPDA)
	{
		...
		{
			BackpackObj = Cast<UItemStorageObject>(EquipObj);
		}
		else if (CPDA->EquipmentType == EEquipmentType::EET_Vest)
		{
			VestObj = Cast<UItemStorageObject>(EquipObj);
		}
	...
	if (EquipObj)
	{
		...
		if (InnerEquipment)
		{
			...
			if (InnerCPDA->ItemName.EqualTo(OuterCPDA->ItemName))
			{
				...
				AEquipment* EquipActor = EquipObj->Equipment;
				if(!EquipActor) return false;
				//remove 직전에 작동중인 Timer가 있다면 timer를 clear하고 지운다.
				GetWorld()->GetTimerManager().ClearAllTimersForObject(EquipActor);				
...
{
	if (!(EquipmentItems.Num() > 0)) return nullptr;
...
void UEquipmentComponent::RemoveAllEquip()
{
	if(!(EquipmentItems.Num() > 0)) return;

	for (UNewItemObject* EquippedObj : EquipmentItems)
	{
		if(!EquippedObj) continue;

		if (EquippedObj->Equipment)
		{
			RemoveEquipment(EquippedObj);
		}
	}
}
///////////////////////////////////////////////////////////////////////////////////////////////////////
